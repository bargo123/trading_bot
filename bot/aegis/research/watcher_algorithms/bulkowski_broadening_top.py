"""Bulkowski broadening-top megaphone and confirmed-breakout perspective."""
...
ALGORITHM_ID = "bulkowski_broadening_top"
...
def evaluate(state):
    ...
    if result["view"] == "MISSING_DATA":
        return result
    if normalized_status(first(state, "bulkowski_broadening_type")) != "top":
        result["reasons"] = ["this perspective requires a broadening top"]
        return result
    if normalized_status(first(state, "bulkowski_broadening_prior_trend")) != "up":
        result["reasons"] = ["a broadening top requires a rising trend into the formation"]
        return result
    ...
    if top_slope <= 0 or bottom_slope >= 0 or top_touches < 2 or bottom_touches < 2 or high <= low:
        result["reasons"] = ["a broadening top needs diverging positive/negative boundaries and two touches on each side"]
        return result
    if not observed_bool(first(state, "bulkowski_broadening_breakout_close_confirmed")):
        result["reasons"] = ["the broadening-top breakout is not confirmed by a close outside the formation"]
        return result
    if (breakout == "UP" and breakout_price <= high) or (breakout == "DOWN" and breakout_price >= low):
        result["reasons"] = ["the confirmed close is not outside the broadening-top range"]
        return result
    height = high - low
    result.update({
    ...
    return finish(result, state, "BUY" if breakout == "UP" else "SELL", "a rising-trend megaphone has diverging boundaries and a confirmed range breakout")
