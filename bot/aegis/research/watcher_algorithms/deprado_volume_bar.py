"""López de Prado standard volume-bar construction diagnostic."""
...
ALGORITHM_ID = "deprado_volume_bar"
SOURCES = ("Marcos López de Prado — Advances in Financial Machine Learning",)
KEYS = ("deprado_standard_prices", "deprado_standard_volumes", "deprado_standard_bar_size", "deprado_standard_bar_data_provenance")


def evaluate(state):
    prices = positive_series(state, "deprado_standard_prices")
    activity = positive_series(state, "deprado_standard_volumes")
    bar_size = number(first(state, "deprado_standard_bar_size"))
    found = values(state, *KEYS)
    missing = []
    for key, value in (("deprado_standard_prices", prices), ("deprado_standard_volumes", activity), ("deprado_standard_bar_size", bar_size)):
        if value is None:
            missing.append(key)
    ...
    if len(prices) != len(activity) or bar_size <= 0:
        ...
        result["reasons"] = ["volume-bar inputs must be aligned and positive"]
        return result
    ...
    result["deprado_standard_bar_assessment"] = "VOLUME_BARS_MEASURED"
    result["warnings"] = ["volume bars structure observations; they do not authorize a trade"]
    return result
