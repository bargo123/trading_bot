"""Bulkowski right-angled descending broadening formation perspective."""
...
ALGORITHM_ID = "bulkowski_right_angled_descending"
...
def evaluate(state):
    ...
    if result["view"] == "MISSING_DATA":
        return result
    if normalized_status(first(state, "bulkowski_right_angled_type")) != "descending":
        result["reasons"] = ["this perspective requires a right-angled descending formation"]
        return result
    ...
    if abs(horizontal) > 0.02 or sloped >= 0 or horizontal_touches < 2 or sloped_touches < 2 or high <= low:
        result["reasons"] = ["the descending right-angled formation needs a horizontal top and falling lower boundary with two touches each"]
        return result
    if not observed_bool(first(state, "bulkowski_right_angled_breakout_close_confirmed")):
        result["reasons"] = ["the right-angled descending breakout is not confirmed by a close outside the formation"]
        return result
    if (breakout == "UP" and breakout_price <= high) or (breakout == "DOWN" and breakout_price >= low):
        result["reasons"] = ["the confirmed close is not outside the right-angled descending range"]
        return result
    height = high - low
    ...
    return finish(result, state, "BUY" if breakout == "UP" else "SELL", "a horizontal-top, falling-bottom formation produced a confirmed breakout")
