"""Bulkowski ascending broadening-wedge structure and breakout perspective."""
...
ALGORITHM_ID = "bulkowski_ascending_broadening_wedge"
...
def evaluate(state):
    ...
    if result["view"] == "MISSING_DATA":
        return result
    if normalized_status(first(state, "bulkowski_broadening_wedge_type")) != "ascending":
        result["reasons"] = ["this perspective requires an ascending broadening wedge"]
        return result
    ...
    if None in (upper, lower, upper_touches, lower_touches, breakout_price, high, low) or breakout is None:
        ...
        return result
    if not upper > lower > 0 or upper_touches < 3 or lower_touches < 3 or high <= low:
        result["reasons"] = ["an ascending broadening wedge needs two rising, diverging boundaries and three touches on each side"]
        return result
    if not observed_bool(first(state, "bulkowski_broadening_wedge_breakout_close_confirmed")):
        result["reasons"] = ["the ascending broadening-wedge breakout is not confirmed"]
        return result
    if (breakout == "UP" and breakout_price <= high) or (breakout == "DOWN" and breakout_price >= low):
        result["reasons"] = ["the confirmed close is not outside the ascending broadening-wedge range"]
        return result
    height = high - low
    ...
    return finish(result, state, "BUY" if breakout == "UP" else "SELL", "two rising, diverging boundaries and a confirmed breakout identify the ascending broadening wedge")
