...
def evaluate(state):
    ...
    if not observed_bool(first(state, "bulkowski_broker_event_announced")) or None in (move, average, high, low, price) or breakout is None or high <= low:
        result["reasons"] = ["an upgrade event and finite announcement range are required"]
        return result
    if move <= average or not observed_bool(first(state, "bulkowski_broker_event_volume_above_average")) or not observed_bool(first(state, "bulkowski_broker_event_breakout_close_confirmed")):
        result["reasons"] = ["an upgrade requires an above-average high-volume reaction and confirmed break"]
        return result
    if (breakout == "UP" and price <= high) or (breakout == "DOWN" and price >= low):
        result["reasons"] = ["the upgrade event breakout is not outside its announcement range"]
        return result
    height = high - low
    ...
    return finish(result, state, "BUY" if breakout == "UP" else "SELL", "a stock upgrade produced a large high-volume directional break")
