"""Bulkowski diamond-bottom widening/narrowing and breakout perspective."""
...
ALGORITHM_ID = "bulkowski_diamond_bottom"
...
def evaluate(state):
    ...
    if result["view"] == "MISSING_DATA":
        return result
    if normalized_status(first(state, "bulkowski_diamond_type")) != "bottom":
        result["reasons"] = ["this perspective requires a diamond bottom"]
        return result
    ...
    if normalized_status(first(state, "bulkowski_diamond_prior_trend")) != "down":
        result["reasons"] = ["a diamond bottom requires a declining trend into the pattern"]
        return result
    ...
    return finish(result, state, "BUY" if breakout == "UP" else "SELL", "a declining-trend diamond widened, narrowed, and confirmed outside its boundary")
