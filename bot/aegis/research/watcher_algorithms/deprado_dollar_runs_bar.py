"""López de Prado dollar-runs-bar event diagnostic."""
...
def evaluate(state):
    ...
    amounts = positive_series(state, "deprado_dollar_run_values")
    ...
    for key, value in (
        ...
        ("deprado_dollar_run_values", amounts),
    ...
    if len(prices) != len(amounts) or bar_size <= 0 or not 0 <= buy_probability <= 1 or buy_mean <= 0 or sell_mean <= 0:
        ...
        result["reasons"] = ["dollar runs inputs must be aligned, positive, and bounded"]
        return result
    ...
    result["deprado_dollar_runs_expected"] = expected
    result["deprado_dollar_runs_events"] = events
    result["deprado_dollar_runs_event_count"] = len(events)
    result["deprado_dollar_runs_assessment"] = "DOLLAR_RUN_EVENTS_MEASURED"
    result["warnings"] = ["dollar runs bars sample one-sided activity; they do not authorize a trade"]
    return result
