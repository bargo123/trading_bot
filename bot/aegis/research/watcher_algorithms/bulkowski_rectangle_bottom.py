"""Bulkowski rectangle bottom: horizontal range after a declining trend."""
...
ALGORITHM_ID = "bulkowski_rectangle_bottom"
SOURCES = ("Thomas N. Bulkowski — Encyclopedia of Chart Patterns",)
SOURCE_PAGES = "563-570"
KEYS = (
...
def evaluate(state):
    ...
    if normalized_status(first(state, "bulkowski_rectangle_type")) != "bottom" or normalized_status(first(state, "bulkowski_rectangle_prior_trend")) != "down":
        result["reasons"] = ["a rectangle bottom requires a prevailing declining trend"]
        return result
    ...
    return finish(result, state, "BUY" if breakout == "UP" else "SELL", "a declining-trend rectangle supplied two-sided horizontal support and resistance")
