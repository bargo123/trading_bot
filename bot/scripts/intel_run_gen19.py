#!/usr/bin/env python3
"""GEN19: unused skip on 30d leftovers vs five-flag live combo.

Live: rsi_ext + doji_against + stretched_doji_buy + barbwire_sell + late_buy_chase.
Do not re-queue chop_doji or barbwire_buy. Do not target poison 19:20 or hydra-trap 09:48.
...
def _asia_hour_5_stretch_buy(rec: dict[str, Any]) -> bool:
    if rec.get("side") != "buy":
        return False
    hour = _hour(rec)
...
SKIP_IDEAS: list[dict[str, Any]] = [
    {
        ...
        "hypothesis": (
            ...
            "Asia midnight. Not hour-4/21, not london_dead_er 7/8/11/12."
        ),
    },
    {
        "id": "intel_asia_hour_5_stretch_buy",
        "kind": "book",
        "weakness": "exhaustion",
        "pred": _asia_hour_5_stretch_buy,
        "patch": {**LIVE, "intel_skip_asia_hour_5_stretch_buy": True},
        "hypothesis": (
            "Skip BUY at UTC hour 5 when loc 0.80–1.0 and EMA stretch >=2 pips. "
            "Hits Jul 7 05:02. Not loc>1 hydra cousin, not late_buy RSI 65–70, not hour-4 sell."
        ),
    },
]
...
def main() -> None:
    ...
    live_e = None
    picked = None
    out_path = INTEL_DIR / "gen19_report.json"
    dump_preview = {
        "generation": "gen19",
        "bars": int(len(df)),
        "lookback_days": 30,
