#!/usr/bin/env python3
"""GEN18: unused skip on 30d leftovers vs five-flag live combo.

Live: rsi_ext + doji_against + stretched_doji_buy + barbwire_sell + late_buy_chase.
...
Does not edit YAML unless this chat promotes after both gates beat.
"""
...
def _barbwire_buy(rec: dict[str, Any]) -> bool:
    return rec.get("side") == "buy" and bool((rec.get("features") or {}).get("brooks_barbwire"))
...
SKIP_IDEAS: list[dict[str, Any]] = [
    {
        "id": "intel_barbwire_buy",
        "kind": "book",
        "weakness": "barbwire",
        "pred": _barbwire_buy,
        "patch": {**LIVE, "intel_skip_barbwire_buy": True},
        "hypothesis": (
            "Brooks barbwire BUY only. Mirror of live barbwire_sell. "
            "Hits overlapping-bar buy sprays. Not global barbwire WAIT, not 09:48."
        ),
    },
    {
        "id": "intel_chop_doji",
        ...
        "hypothesis": "Unused WAIT: Volman doji only when ER<0.20. Not doji_against (body with).",
    },
    {
        ...
        "hypothesis": "Unused: London hours 7/8/11/12 when ER<0.10. Not hour-12 sell rename.",
    },
    {
        ...
        "hypothesis": "Unused WAIT: skip bar when ATR > 1.6x SMA. Not poison/hydra.",
    },
]
...
def main() -> None:
    ...
    live_e = None
    picked = None
    out_path = INTEL_DIR / "gen18_report.json"
    dump_preview = {
        "generation": "gen18",
        "bars": int(len(df)),
        "lookback_days": 30,
    ...
    summary = {
        ...
        "verdict": verdict,
        "path": path,
        "gate": "Promote only if OOS E AND full $ both beat the five-flag live combo. Do not target poison 19:20 or hydra-trap 09:48. Do not enable asia_hour_4_sell. Replay is not the gate. Small-n. Not 100% WR.",
