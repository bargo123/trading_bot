#!/usr/bin/env python3
"""GEN13: path-test floor_run_sell around the four-flag live combo.

Replay is not the gate (gen12 hour-19: 1/4 replay, 35 path-dependent winners).
...
FLOOR_RUN = {
    ...
    "hypothesis": "Skip SELL at loc<=0.15 after ER>=0.40 run. Opposite of floor-chop. Not stretched_sell. Replay is not the gate.",
}
...
def _dump_loss(r: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "features": f,
        "hits": {"floor_run_sell": _floor_run_sell(r)},
    }
...
def main() -> None:
    ...
    scan = {"intel_floor_run_sell": _scan(losses, wins, _floor_run_sell)}
    print(
    ...
    hits = scan["intel_floor_run_sell"]
    pick = FLOOR_RUN if hits["sl_hits"] >= 1 else None
    ...
    live_e = None
    if pick is None:
        print("floor_run_sell no-op on remaining SLs; skip generation", flush=True)
    else:
        print(
            f"path-test floor_run_sell replay={hits} (replay is not the gate)",
            flush=True,
        )
        out = run_generation(
            df,
        ...
        for r in out["experiments"]:
            ...
            row["winner_sacrifice_replay"] = hits
    ...
    summary = {
        ...
        "bars": int(len(df)),
        "generation": "gen13",
        "gate": "Promote only if OOS E AND full $ both beat the four-flag live combo. Replay is not the gate. Hydra honesty. Small-n. Not 100% WR.",
        "live_combo": {
            "flags": [
                "intel_skip_rsi_ext",
    ...
    path = INTEL_DIR / "gen13_report.json"
