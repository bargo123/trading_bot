#!/usr/bin/env python3
"""GEN12: one unused skip around the four-flag live combo.

Hits a real leftover with a combo that is not a rename of a rejected gate
(no loc>1 buy, skip-NaN, stretched_sell, floor-chop, wrong_edge).
Promote only if OOS E AND full $ beat the live combo. Never mt5.shutdown().
...
def _floor_run_sell(rec: dict[str, Any]) -> bool:
    if rec.get("side") != "sell":
        return False
    loc = _num(rec, "range_loc")
    er = _num(rec, "kaufman_er")
    return loc is not None and er is not None and loc <= 0.15 and er >= 0.40


def _ny_hour_19_sell(rec: dict[str, Any]) -> bool:
    if rec.get("side") != "sell":
        return False
...
IDEAS: list[dict[str, Any]] = [
    {
        "id": "intel_floor_run_sell",
        "pred": _floor_run_sell,
        "kind": "book",
        "weakness": "late_entry",
        "patch": {**LIVE, "intel_skip_floor_run_sell": True},
        "hypothesis": "Skip SELL at loc<=0.15 after ER>=0.40 run. Opposite of floor-chop. Not stretched_sell.",
    },
    {
        "id": "intel_ny_hour_19_sell",
        "pred": _ny_hour_19_sell,
        "kind": "book",
        "weakness": "chop",
        "patch": {**LIVE, "intel_skip_ny_hour_19_sell": True},
        "hypothesis": "Skip SELL in UTC hour 19. Session, not stretched_sell / london_hour_12.",
    },
]
...
def _dump_loss(r: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "features": f,
        "hits": {
            "floor_run_sell": _floor_run_sell(r),
            "ny_hour_19_sell": _ny_hour_19_sell(r),
        },
    }
...
def main() -> None:
    ...
    live_e = None
    if pick is None:
        print("no unused skip hits remaining real SLs; skip generation", flush=True)
    else:
    ...
    summary = {
        ...
        "bars": int(len(df)),
        "generation": "gen12",
        "gate": "Promote only if OOS E AND full $ both beat the four-flag live combo. Hydra $ without losses_avoided allowed but both gates required. Not loc>1 / skip-NaN / stretched_sell. Small-n. Not 100% WR.",
        "live_combo": {
            "flags": [
                "intel_skip_rsi_ext",
    ...
    path = INTEL_DIR / "gen12_report.json"
